...
define([
    ...
    $.widget('mage.blogCategoryTree', {
        ...
        _create: function () {
            ...
            this._bind();
            this.element.find(this.options.initSelector).jstree(this.options.treeConfig);
        },
        ...
        _getNodesPositionData: function (currentNode) {
            var tree = this.element.find(this.options.initSelector),
                prevNode = tree.jstree('get_node', tree.jstree('get_prev_dom', currentNode, true)),
                parentNode = currentNode.parent !== '#' ? tree.jstree('get_node', currentNode.parent) : false,
            ...
            while (node && node.id) {
                ...
                if (!_.isEmpty(node.children)) {
                    firstChildNode = tree.jstree('get_node', _.first(node.children));
                    data = _.union(data, this._getNodesPositionData(firstChildNode));
                }
                prevNode = node;
                node = tree.jstree('get_node', tree.jstree('get_next_dom', node, true));
            }
