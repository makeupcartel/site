...
Class CheckStoreCredit implements ObserverInterface
{
    /**
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        try{
            ...
            if ($referral_id) 
            {
                $balanceCustomer = $this->_balanceFactory->create()->setCustomer($customer)
                ->setWebsiteId($customer->getWebsiteId())
                ->setAmountDelta($signup_bonus)->setComment('store credit from referal');
                $balanceCustomer->save();
                $currencySymbol = $this->priceHelper->currency($signup_bonus, true, false);
                $this->messageManager->addSuccessMessage(__('Congratulations! you have received store credit of %1', $currencySymbol));
                $referralCustomer = $this->customerModel->load($referral_id);
                $balanceCustomerReferal = $this->_balanceFactory->create()->setCustomer($referralCustomer)
                ->setWebsiteId($referralCustomer->getWebsiteId())
                ->setAmountDelta($referral_reward)->setComment('store credit from referal');
                $balanceCustomerReferal->save();
            }
