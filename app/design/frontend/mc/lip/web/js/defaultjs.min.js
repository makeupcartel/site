define([
    ...
    //     }
    // })<script>
 //move in filter sidebar of category
        jQuery('.catalog-category-view .sidebar-main #layered-filter-block .filter-content-area .filter-options-item').prependTo('.catalog-category-view .sidebar-main #layered-filter-block .filter-content-area #narrow-by-list');
    AOS.init({
        duration: 1200,
    });
    $(document).ready(function() {
        ...
        $(document).on('click', '.edit-field-btn', function() {
            ...
            $('label.locked[for="' + id + '"]').removeClass('locked').trigger('click');
        });
        ...
        if (width < 768) {
            ...
            jQuery('.header--right--wrapper .block.block-search').prependTo('.nav-sections .nav-sections-items');

        }

        if (width > 768) {
            if (jQuery('.nav-sections nav-sections-items').find('.block-search')) {
                jQuery('.nav-sections .nav-sections-items .block.block-search').prependTo('.header--right--wrapper');
            }
        ...
        // .ampromo
        setTimeout(function() {
            alignHeight('.checkout-cart-index .ampromo-items-content .ampromo-title');
            alignHeight('.checkout-cart-index .ampromo-items-form .product-info-price');
            alignHeight('.checkout-cart-index .ampromo-items-content .ampromo-item-image');
            alignHeight('.checkout-cart-index .ampromo-items-content .align-swatch');
        }, 500);
    ...
    $(document).ready(function() {
        ...
        /* update cart when subscription change */
        /* $(document).on('click', '.additional-info-wrapper .subscription-container .field.choice input[type="radio"]:not(:checked) + label', function () {
             $('.cart.main.actions .action.update').trigger('click');
         });*/
        var currentSelect = ''
    ...
    // MOVE FOOTER RESPONSIVE
    jQuery(document).ready(function() {
        var flag = true;
        var width = jQuery(window).width();
        if (width < 768) {
            jQuery('.footer-middle-bottom').appendTo('.footer--middle--right');
            jQuery('.footer--middle--left .title-content').hide();
            jQuery('.page-footer .footer--bottom div ul li.copyright').appendTo('.page-footer .footer--bottom div ul');
            if (flag) {
                jQuery('.footer.content .footer--middle--left h4').click(function() {
        ...
        if (width >= 768) {
            jQuery('.footer--middle--left .title-content').show();
            jQuery('.footer-middle-bottom').appendTo('.footer.content');
            jQuery('.page-footer .footer--bottom div ul li.copyright').prependTo('.page-footer .footer--bottom div ul');
        }
        jQuery(window).resize(function() {
            var width = jQuery(window).width();
            if (width < 768) {
