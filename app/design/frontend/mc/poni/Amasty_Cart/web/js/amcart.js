    ...
    $.widget('mage.amCart', {
            ...
            element.on('click', function (e) {
                if (!$('body').hasClass('wishlist-index-index')) {
                    ...
                    if (dataPost) {
                        ...
                        if(parent.find('div.swatch-attribute').length){
                            parent.find('div.swatch-attribute').each(function(){
                                if(typeof $(this).attr('option-selected') !== undefined && ($(this).attr('option-selected') !== '' || $(this).attr('option-selected') !== null)){
                                    form.append(
                                        $('<input />', {name: 'super_attribute['+$(this).attr('attribute-id')+']', type: 'hidden', value: $(this).attr('option-selected')})
                                    );
                                }
                            })
                        }
        ...
        submitForm: function (form) {
            ...
            if (form.has('input[type="file"]').length && form.find('input[type="file"]').val() !== '') {
                ...
                if (!$('body').hasClass('quickorder-index-index') && !$('body').hasClass('wishlist-index-index')) {
        ...
        ajaxSubmit: function (form) {
            ...
            * */
            ...
            var urlCheck = self.options['send_url'];
            var redirects = form.closest('.product-item').find('.product-item-photo').attr('href');
            var checkUrl = url.search("UpdateItemOptions");
            if (checkUrl != -1 && checkUrl != "" && checkUrl != null && checkUrl != 0) {
                ...
                success: function (response) {
                    ...
                    if (response.error) {
                        ...
                        try {
                            ...
                            var checkrelated = $("div#confirmBox .am-related-box").find(".block.related").length;
                            if (checkrelated == 0) {
                                $("div#confirmBox").addClass("empty_ralated");
                            }
                    ...
                    var a = $("body").find("div#confirmBox").length;
                    if (a != 0) {
                        $("body").addClass("popup_mobile");
                    }


                    var checkUrl2 = urlCheck.search("UpdateItemOptions");
                    if (checkUrl2 != -1 && checkUrl2 != "" && checkUrl2 != null && checkUrl2 != 0) {
                        $('#confirmBox .ajax-added-item .text').css('display', 'none');
                        $('#confirmBox .ajax-added-item .text.update').css('display', 'block');
                    }
        ...
        showAnimation: function (form) {
            ...
            if (loadingType == 0 || !foundImage) {
                var progress = $('<div />', {id: "amprogress"}),
                    ...
                    img = $('<div class="lds-spinner"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div>');
        ...
        confirm: function (params) {
            var wrapper = $('#confirmBox, #confirmOverlay, .confirmBox-wrapper');
            if (wrapper.length > 0) {
                wrapper.remove();
            }
            var buttonHTML = '',
            ...
            var titlePopup = $('<p class="titlepopup"></p>').html('Added to Cart');
            var cross = $('<span title="' + $.mage.__("Close") + '" class="cross"></span>').html('&times;');
            cross.click(function () {
                self.confirmHide();
                $("body").removeClass("popup_mobile");
            });
            confirmBox.append(titlePopup);
            ...
            if ($(window).width() < 768) {
                confirmBox.appendTo($('body')).wrap("<div class='confirmBox-wrapper'></div>");
                $('.confirmBox-wrapper').addClass('slide-effect')
                setTimeout(function () {
                    $('.confirmBox-wrapper').addClass('slide-effect-complete')
                }, 500);
            } else {
                confirmBox.hide().appendTo($('body')).wrap("<div class='confirmBox-wrapper'></div>").fadeIn();
            }
        ...
        confirmHide: function () {
            if ($(window).width() < 768) {
                $('.confirmBox-wrapper').removeClass('slide-effect-complete');
            }
            $('#confirmBox, #confirmOverlay').fadeOut(function () {
                $(this).remove();
                $('.confirmBox-wrapper').remove();
