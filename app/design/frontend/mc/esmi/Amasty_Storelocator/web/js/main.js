    ...
    $.widget('mage.amLocator', {
        ...
        makeAjaxCall: function(position) {
            ...
            $.ajax({
                ...
                self.Amastyload_search(response);
				jQuery('.amlocator_store_list > div > span').each(function(){
					if (jQuery(this).children('.gold-btn').length > 0) {
						jQuery(this).closest('span').prependTo('.amlocator_store_list > div');
					}
				})
            }));
        },
		...
		Amastyload_search: function(response) {
            this.initializeMap_search();
            this.processLocation(this.options.jsonLocations);
        ...
        initializeMap: function() {
            ...
            // custom set center as Australia
            var thisMap = this.map[this.options.mapId];
            google.maps.event.addListenerOnce(thisMap, 'tilesloaded', function() {
              thisMap.setZoom(2);
              thisMap.setCenter(new google.maps.LatLng(-37.814,144.96332));
            });
            // end custom
        },
		initializeMap_search: function() {
            this.infowindow = [];
            this.marker[this.options.mapId] = [];
            var myOptions = {
                zoom: 9,
